...
import { currentValue, monthlyValue, taxBreakdown, totalCoupons, type ReferenceRatesLike } from "@/lib/investments";
...
function CouponRow({ id, date, amount }: { id: string; date: Date; amount: number }) {
  ...
  return (
    <div className="flex items-center justify-between py-1.5">
      <span className="font-mono text-[12px] text-[var(--color-muted-2)]">{formatDate(date)}</span>
      <div className="flex items-center gap-3">
        <span className="font-mono text-[12.5px] font-medium text-[#0c9e57]">+{formatCurrency(amount)}</span>
        <button
          type="button"
          onClick={() => setEditing(true)}
          className="text-xs font-semibold text-[var(--color-muted-2)] hover:text-[var(--color-ink)]"
...
function CouponAddForm({ investmentId }: { investmentId: string }) {
  ...
  return (
    <form ref={formRef} action={formAction} className="flex items-end gap-2 pb-3">
      <div>
        <label className="mb-1 block text-[10px] font-bold tracking-wide text-[var(--color-muted-2)] uppercase">
          Date
        </label>
        <input
          name="date"
      ...
      {state.error && <span className="text-[11px] text-[#dc3545]">{state.error}</span>}
    </form>
  );
}
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <div>
          <p className="font-mono text-[10px] tracking-wide text-[var(--color-muted-2)] uppercase">Valor atual</p>
          <p className="font-mono text-sm font-semibold text-[var(--color-ink)]">{formatCurrency(value)}</p>
        </div>
        <div>
          <p className="font-mono text-[10px] tracking-wide text-[var(--color-muted-2)] uppercase">Investido</p>
          <p className="font-mono text-sm font-semibold text-[var(--color-ink)]">
            {formatCurrency(holding.amountInvested)}
          </p>
        </div>
        <div>
          <p className="font-mono text-[10px] tracking-wide text-[var(--color-muted-2)] uppercase">Ganho bruto</p>
          <p
        ...
        <div>
          <p className="font-mono text-[10px] tracking-wide text-[var(--color-muted-2)] uppercase">Ganho líquido*</p>
          <p
            className="font-mono text-sm font-semibold"
        ...
        <div className="h-40 w-full">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={monthlyData}>
              ...
              <XAxis
                dataKey="label"
                tick={{ fontSize: 10.5, fill: "#9ca3af", fontFamily: "var(--font-dm-mono)" }}
                axisLine={false}
                tickLine={false}
              />
